...
for idxCond = 1:length(cond)
    for iVal = 1:length(validity)
        parfor j=1:200
            if exist(['/network/lustre/iss01/home/zoe.brusselle/fit/p' num2str(pIdx) '_cond' cond{idxCond} '_' validity{iVal} '/res/launcher_2_' num2str(j) '.m'], 'file')
                if ~ exist(['/network/lustre/iss01/home/zoe.brusselle/fit/p' num2str(pIdx) '_cond' cond{idxCond} '_' validity{iVal} '/res/output_SecndPass_' num2str(j) '.mat'], 'file')
                    disp(['RUN p' num2str(pIdx) '_cond' cond{idxCond} '_' validity{iVal} ' launcher_2_' num2str(j)]);
                    run(['/network/lustre/iss01/home/zoe.brusselle/fit/p' num2str(pIdx) '_cond' cond{idxCond} '_' validity{iVal} '/res/launcher_2_' num2str(j) '.m'])
                end
            end
        end
    end
...
for idxCond = 1:length(cond)
    for iVal = 1:length(validity)
        if exist(['/network/lustre/iss01/home/zoe.brusselle/fit/p' num2str(pIdx) '_cond' cond{idxCond} '_' validity{iVal} '/res/output_SecndPass_200.mat'], 'file')
